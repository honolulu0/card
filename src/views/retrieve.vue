...
<template>
  <div class="login-wrap">
    ...
    >
      ...
      <el-form-item label="手机验证码" prop="sms">
        <el-col :span="8">
          <el-input v-model="ruleForm.sms"></el-input>
        </el-col>
...
export default {
  ...
  methods: {
    getAuthCode: function () {
      let res = this.$refs["ruleForm"].validateField("phone", (emailError) => {
        if (emailError) {
          return false;
        }
        this.getAuthCode_();
    ...
    getAuthCode_: function () {
      Api.retrieve_phone({ phone: this.ruleForm.phone }).then((data) => {
        if (data.code == 1) {
          this.$message({
            message: "短信发送成功",
            type: "success",
    ...
    doRegister_: function () {
      Api.retrieve(this.ruleForm).then((data) => {
        if (data.code == 1) {
          this.$message({
            message: "修改成功，请登录",
            type: "success",
